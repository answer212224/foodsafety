...
|--------------------------------------------------------------------------
|
| 解決Undefined method 'can'.intelephense(P1013)問題
| @method can() @see \Spatie\Permission\Traits\HasRoles::can()
|
*/
...
Route::get('/home', function () {
    // 假如有execute-task權限就導向v2任務列表
    if (auth()->check() && auth()->user()->can('execute-task')) {
        return redirect()->route('v2.app.tasks.index');
    } else if (auth()->check() && auth()->user()->can('execute-quality-task')) {
        return redirect()->route('v2.app.quality-tasks.index');
    } else if (auth()->check() && auth()->user()->can('view-record-meal')) {
        // v2採樣紀錄
        return redirect()->route('v2.data.foodsafety.record.meals.index');
    } else if (auth()->check()) {
        // 將使用者登出
        auth()->logout();
        // 回傳403
        return response('請聯絡管理員開通給您角色權限，徐雅苓 3810288 分機 276 或 温惠婷 分機 275，謝謝。', 403);
    } else {
        return redirect()->route('login');
    }
})->name('home');
...
Route::prefix('v1')->middleware(['auth', 'log.user.activity'])->group(function () {
    ...
    Route::prefix('data')->group(function () {
        ...
        Route::prefix('row-data')->group(function () {
            ...
            Route::get('/quality-defect', [RowDataController::class, 'rowDataQualityDefect'])->name('row-data-quality-defect');
            Route::get('/quality-clear-defect', [RowDataController::class, 'rowDataQualityClearDefect'])->name('row-data-quality-clear-defect');
        });
...
Route::prefix('api')->middleware(['auth', 'log.user.activity'])->group(function () {
    ...
    // 舊的路由有問題，所以新增一個新的
    Route::post('/projects/store', [ApiController::class, 'storeProject']);
    // 更新專案資料庫資料
    Route::put('/projects/{project}', [ApiController::class, 'updateProject']);
    // 舊的路由有問題，所以新增一個新的
    Route::put('/projects/update/{project}', [ApiController::class, 'updateProject']);
    // 取得食安缺失資料庫資料
