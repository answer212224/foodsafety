...
class QualityClearDefectsImport implements ToCollection
{
    /**
    ...
    public function collection(Collection $collection)
    {
        ...
        // 額外新增2筆資料
        $collection->push(['effective_date' => $collection[0]['effective_date'], 'main_item' => '待確認', 'sub_item' => '待確認', 'deduct_point' => -2]);
        $collection->push(['effective_date' => $collection[0]['effective_date'], 'main_item' => '其他', 'sub_item' => '其他', 'deduct_point' => -2]);
        ...
        // 刪除該月份的食安缺失資料
        // \App\Models\QualityClearDefect::whereYear('effective_date', $collection[0]['effective_date'])->whereMonth('effective_date', $collection[0]['effective_date'])->delete();

        // 新增該月份的食安缺失資料
        foreach ($collection as $item) {
            $defect = new \App\Models\QualityClearDefect();
            $defect->fill($item);
            $defect->save();
        }
    }
