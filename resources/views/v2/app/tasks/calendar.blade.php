                                        ...
                                        <v-card-actions>
                                            ...
                                            @can('update-task')
                                                ...
                                                <v-btn v-show="selectedEvent.category=='清潔檢查'" text color="blue darken-1"
                                                    :href="'/v2/app/task/' + selectedEvent.id + '/clear-defect/edit'">
                                                    編輯缺失
                                                </v-btn>
                                            @endcan
                                            <v-spacer></v-spacer>
                    ...
                    <v-dialog v-model="importDialog" max-width="500px">
                        <v-card>
                            <v-card-title>
                                ...
                                <v-spacer></v-spacer>
                                <a href="{{ asset('storage/task_import.xlsx') }}" download>範例下載</a>
                            </v-card-title>
            ...
            new Vue({
                ...
                methods: {
                    ...
                    exportExcel() {
                        ...
                        const worksheet = workbook.addWorksheet('食安行程')
                        worksheet.columns = [{
                                header: '日期',
                                key: 'date',
                                width: 15
                        ...
                        this.events.forEach((item) => {
                            worksheet.addRow({
                                ...
                                projects: item.projects.map(project => project.description).join(','),
                                category: item.category,
                                start_at: item.start_at,
                                end_at: item.end_at,
                                status: item.status,
                        ...
                        workbook.xlsx.writeBuffer().then((buffer) => {
                            ...
                            a.href = url
                            a.download = '食安行程_' + year + '年' + month + '月.xlsx'
                            a.click()
