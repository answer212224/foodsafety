            ...
            <v-main class="grey lighten-4">
                <v-container>
                        ...
                        <v-col cols="12" sm="4" v-for="task in tasks" :key="task.id">
                            <v-card>
                                ...
                                <v-card-text>
                                    ...
                                    <v-row>
                                        <div class="subtitle-2 col-12 col-sm-6">
                                            <v-icon small>mdi-timer-play-outline</v-icon>
                                            <span class="text--primary">
                                                @{{ task.start_at }}
                                            </span>
                                        </div>
                                        <div class="subtitle-2 col-12 col-sm-6">
                                            <v-icon small>mdi-timer-off-outline</v-icon>
                                            <span class="text--primary">
                                                @{{ task.end_at }}
                                            </span>
                                        </div>
                                ...
                                <v-card-actions>
                                    <v-spacer></v-spacer>
                                    <v-menu v-if="task.category !== '食材/成品採樣'" :close-on-content-click="false"
                                        ...
                                        <v-list>
                                            <v-list-group prepend-icon="mdi-file-document-edit-outline">
                                                ...
                                                {{-- 新增食安缺失頁面 --}}
                                                <v-list-item :href="`/v2/app/quality-task/${task.id}/defect/create`"
                                                    v-show="task.category === '食安巡檢'"
                                                    :disabled="!(new Date(task.task_date).toISOString().substring(0, 10) ==
                                                        '{{ now()->toDateString() }}')">
                                                    <v-list-item-title>新增食安缺失</v-list-item-title>
                                                </v-list-item>
                                                {{-- 食安稽核紀錄 --}}
                                                <v-list-item :href="`/v2/app/quality-task/${task.id}/defect/edit`"
                                                    v-show="task.category === '食安巡檢'">
                                                    <v-list-item-title>食安稽核紀錄</v-list-item-title>
                                                </v-list-item>
                                                {{-- 新增清檢缺失頁面 --}}
                                                <v-list-item
                                                    :href="`/v2/app/quality-task/${task.id}/clear-defect/create`"
                                                ...
                                                {{-- 清檢稽核紀錄 --}}
                                                <v-list-item
                                                    :href="`/v2/app/quality-task/${task.id}/clear-defect/edit`"
                                                    v-show="task.category === '清潔檢查'">
                                            ...
                                            <v-list-group prepend-icon="mdi-file-pdf-box">
                                                ...
                                                {{-- 報告 --}}
                                                <v-list-item :href="`/v2/app/quality-task/${task.id}/report`"
                                                    target="_blank">
                    ...
                    <v-dialog v-model="mealDialog" max-width="500px">
                        <v-card>
                            <v-card-title>
                                <span class="headline">食材/成品採樣</span>
                                <v-spacer></v-spacer>
                                {{-- 採樣單excel --}}
                                <v-btn color="blue darken-1" text small target="_blank"
                                    :href="taskItem && `/v1/app/task/${taskItem.id}/quality-meal/export`">
                    ...
                    {{-- 主管簽核 dialog --}}
                    <v-dialog v-model="approvalDialog" max-width="300px">
                        <v-card>
                            <v-card-title>
                            ...
                            <v-card-text>
                                <v-container>
                                    <v-row>
                                        <v-col cols="12" sm="12">
                                            <v-text-field label="缺失核對主管" v-model.lazy="taskItem.inner_manager"
        ...
        <script>
            new Vue({
                ...
                methods: {
                    getTasks() {
                        this.loading = true
                        axios.get("/api/user/quality-tasks", {
                                params: {
                                    status: this.status,
                                    limit: this.limit
                                }
                    ...
                    changeStatus(taskId, isCompleted) {
                        this.loading = true
                        axios.put(`/api/user/quality-tasks/${taskId}`, {
                    ...
                    // 確認此任務所有人員都已完成
                    isAllCompleted(task) {
                        axios.get(`/api/quality-tasks/${task.id}/is-all-completed`)
                            .then(response => {
                    ...
                    saveMealIsTaken() {
                        this.loading = true
                        axios.put(`/api/quality-tasks/${this.taskItem.id}/meals`, {
                    ...
                    saveBoss() {
                        this.loading = true
                        axios.put(`/api/quality-tasks/${this.taskItem.id}/boss`, {
