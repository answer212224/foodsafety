                            ...
                            <v-row align="center" class="pa-2">
                                ...
                                <v-col cols="12" sm="6">
                                    <v-text-field v-model="search" append-icon="mdi-magnify"
                                        label="啟動月份、品牌店代碼、品牌、店別、類別、廚別、區站、編號、名稱、備註、檢項、檢樣項目" single-line></v-text-field>
                                </v-col>
                            </v-row>
                        </template>
                ...
                methods: {
                    ...
                    save() {
                        ...
                        if (this.editedIndex > -1) {
                            axios.put('/api/quality-meals/' + this.editedItem.id, this.editedItem)
                                .then(response => {
                                    alert('更新成功')
                                    this.getMeals()
                                })
                            ...
                            axios.post('/api/quality-meals', this.editedItem)
                                .then(response => {
                                    alert('新增成功')
                                    this.getMeals()
                                })
                    ...
                    deleteItem(item) {
                        if (confirm('確定要刪除嗎?')) {
                            this.loading = true
                            axios.delete('/api/quality-meals/' + item.id)
                                .then(response => {
                                    alert('刪除成功')
                                    this.getMeals()
                                })
                    ...
                    importMeals() {
                        ...
                        axios.post('/api/quality-meals/import', formData, {
                                headers: {
